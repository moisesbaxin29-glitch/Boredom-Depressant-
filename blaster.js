...
function update() {
    ...
    // Move projectiles upwards safely
    ...
    // FIXED LOOP STRUCTURE: Safely check and delete elements without crashing indices
    for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        for (let j = projectiles.length - 1; j >= 0; j--) {
            ...
            if (p.x < e.x + e.width && p.x + p.width > e.x && p.y < e.y + e.height && p.y + p.height > e.y) {
                // Remove enemy and projectial arrays data blocks cleanly
                enemies.splice(i, 1); 
                ...
                // Break out of the inner loop immediately since this enemy box is gone
                break;
            }
        }
    }
...
function draw() {
    ...
    // Draw Cyan Player Ship Triangle
